...
import { ArrowLeft, Search, Plus, X, Upload, Trash2 } from 'lucide-react';
...
const PartnerDetails = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      const payload = {
        ...
        // specializations could be mapped, but leaving out unless api supports it.
      };
      const orgRes = await apiService.updateOrganization(id, payload);
      ...
      console.log('Final resolved orgId for doctors in detail view:', orgId);
  ...
  const handleAddDoctor = async (doctor) => {
    try {
      ...
      console.log('Hitting /api/doctors/ POST (Detail View) with:', docPayload);
          ...
          <div className="bg-white border border-[#E5E7EB] rounded-2xl p-8 flex flex-col items-center h-full justify-between" style={{ boxShadow: '0px 1px 2px 0px #0000000D' }}>
            <div className="flex flex-col items-center pt-4">
              <div className="w-24 h-24 rounded-full bg-[#94A3B8] text-[#FFFCFB] flex items-center justify-center text-3xl font-serif mb-4 shadow-inner">
                {data.name?.split(' ').map(n => n[0]).join('').substring(0, 3) || data.orgName?.split(' ').map(n => n[0]).join('').substring(0, 3)}
              </div>
