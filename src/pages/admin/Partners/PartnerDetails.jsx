...
import { ArrowLeft, Search, Plus, X, Trash2, Camera, Phone, MapPin, Building2, ChevronDown } from 'lucide-react';
...
const PartnerDetails = () => {
  ...
  const fetchData = React.useCallback(async () => {
    try {
      ...
      setDetails({
        name: orgData.name || orgData.orgName || '',
        ...
        <div className="w-full lg:w-[320px] shrink-0 ">
          ...
          >
            <div className="flex flex-col items-center pt-4">
              <div className="w-24 h-24 rounded-full bg-[#F8FAFC] border border-[#E2E8F0] flex items-center justify-center text-3xl font-serif mb-4 shadow-sm overflow-hidden">
                {data.logo ? (
                  <img
                    src={data.logo}
                    alt={data.name}
                  ...
                  <div className="w-full h-full flex items-center justify-center bg-[#94A3B8] text-white">
              ...
              {isEditing ? (
                <input
                  ...
                  className="text-2xl font-bold text-[#22333B] text-center mb-3 font-serif border-b border-gray-200 focus:outline-none focus:border-[#F68E5F] w-full"
                />
              ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {isEditing ? (
                      <input
                        ...
                        onChange={handleDetailChange}
                        className="text-sm font-semibold border-b border-gray-200 focus:outline-none focus:border-[#F68E5F] w-full bg-transparent"
                      />
                    ) : (
                      <p className="text-sm font-semibold break-words">{data.contact || 'N/A'}</p>
                    )}
                  </div>
                </div>
                <div className="flex items-start gap-3 text-[#4B5563]">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {isEditing ? (
                      <textarea
                        ...
                        onChange={handleDetailChange}
                        rows={2}
                        className="text-sm font-semibold border-b border-gray-200 focus:outline-none focus:border-[#F68E5F] w-full bg-transparent resize-none"
                      />
                    ) : (
                      <p className="text-sm font-semibold whitespace-normal break-words" title={data.location}>{data.location || 'N/A'}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div key={idx} className="bg-[#F8FAFC] border border-[#F1F5F9] rounded-xl p-4 flex gap-4 items-start">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start">
                    <h4 className="text-[15px] font-bold text-[#22333B] mb-0.5 truncate" title={doc.name}>{doc.name}</h4>
                    {isEditing && (
                  ...
                  <p className="text-[10px] font-bold text-[#94A3B8] tracking-wider uppercase mb-3 truncate" title={doc.specialty}>{doc.specialty}</p>
