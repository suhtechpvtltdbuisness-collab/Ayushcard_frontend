...
const StatusBadge = ({ status }) => {
  const styles = {
    Paid: "bg-[#76DB1E33] text-[#76DB1E]",
    Pending: "bg-[#FFA10033] text-[#FFA100]",
  ...
  return (
    <div
      className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold ${activeStyle} items-center justify-center min-w-20`}
    >
      {status}
    </div>
  );
...
const Salary = () => {
  ...
  const handleDeleteConfirm = () => {
    if (itemToDelete) {
      const updatedData = salaries.filter((d) => d.id !== itemToDelete.id);
  ...
  const processedData = useMemo(() => {
    ...
    return result;
  }, [salaries, searchQuery, sortConfig, activeFilter]);
  ...
  const handleExport = () => {
    ...
    exportToCSV(dataToExport, "Salary_Export.csv");
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 shrink-0 gap-4 sm:gap-0">
        <h2 className="text-xl font-bold text-[#22333B]">Salary</h2>
            ...
            <table className="w-full text-left border-collapse relative">
              <thead className="sticky top-0 z-10 bg-[#FFFFFF]">
                <tr>
                  ...
                  {renderSortableHeader("Date", "date", "left", "w-[130px]")}
                  {renderSortableHeader(
                    "Status",
                    "status",
                    "center",
                ...
                {paginatedData.map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3 px-4 text-center font-normal whitespace-nowrap">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-xl p-6 w-100 shadow-lg animate-in fade-in zoom-in duration-200">
            ...
            <p className="text-[#4B5563] text-sm mb-6 pl-12 line-clamp-3">
              Do you really want to delete the salary record for{" "}
