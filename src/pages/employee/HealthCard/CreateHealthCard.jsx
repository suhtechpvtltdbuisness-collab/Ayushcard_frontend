...
const CreateHealthCard = () => {
  ...
  const startCashCamera = async () => {
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      toastError(
        "Camera not supported on this device/browser. Please use Upload from Gallery.",
      );
      return;
    }
    ...
    try {
      ...
      if (
        ...
        toastError(
          "No camera device found. Please connect a camera or use Upload from Gallery.",
        );
      } else {
  ...
  const startCamera = async () => {
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      toastError(
        "Camera not supported on this device/browser. Please use Gallery Upload.",
      );
      return;
    }
    ...
    try {
      ...
      if (
        ...
        toastError(
          "No camera device found. Please connect a camera or use Gallery Upload.",
        );
      } else {
  ...
  const performSave = async (method, resolvedTxnId) => {
    ...
    try {
      ...
      setSaveError(errMsg || "Failed to create health card. Please try again.");
    } finally {
    ...
    <div className="animate-in fade-in slide-in-from-right-4 duration-300 max-w-4xl mx-auto">
      ...
      <div className="mb-4">
        ...
        <p className="text-[#666666] text-[15px]">
          Please upload or scan an identity document of the family head
          (Aadhaar, PAN, etc.)
        </p>
      </div>
      ...
      <div className="flex flex-col gap-4 mb-8 relative w-full items-center justify-center">
        ...
        <div className="w-full border-2 border-[#fa8112]/30 bg-orange-50/20 p-4 sm:p-8 rounded-3xl flex flex-col items-center justify-center transition-all overflow-hidden min-h-[400px]">
          {cameraActive ? (
            ...
            <div className="flex flex-col items-center group">
              ...
              >
                {ocrLoading ? (
                  <Loader2 className="animate-spin text-[#fa8112]" size={36} />
                ) : (
              ...
              <div className="flex flex-col sm:flex-row gap-3 w-full">
                ...
                <button
                  onClick={() =>
                    document.getElementById("ocr-input-create").click()
                  }
        ...
        <div className="w-full border-2 border-[#fa8112]/30 bg-orange-50/40 p-6 rounded-[32px]">
          ...
          <p className="text-[12px] text-gray-600 text-center mb-4">
            Step 1: Scan to auto-fill details (optional). Step 2: Upload a clear
            photo of the same document.
          </p>
      ...
      <div className="mb-6">
        ...
        <div className="space-y-4">
          {(formData.members || []).map((member, index) => (
            <div
              key={index}
              className="bg-white border-2 border-[#E2E8F0] rounded-xl p-5 relative"
            >
              <button
                onClick={() => {
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        newMembers[index].name = e.target.value.replace(
                          /[0-9]/g,
                          "",
                        );
                        setFormData({ ...formData, members: newMembers });
                      }}
                      className="flex-1 border border-[#E2E8F0] rounded-lg px-3 py-2 text-[14px] focus:outline-none focus:border-[#fa8112]"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl mx-auto">
              ...
              >
                <div className="w-14 h-14 bg-orange-50 rounded-2xl flex items-center justify-center mb-4 group-hover:bg-[#fa8112] transition-colors">
                  <UploadCloud
                    size={28}
                    className="text-[#fa8112] group-hover:text-white"
                  />
                </div>
                <h4 className="font-bold text-[#222222] text-[18px] mb-2">
                  Online Payment
                </h4>
                <p className="text-sm text-gray-500 mb-6">
                  Secure checkout via UPI, Cards, or Netbanking. Fast and
                  instant confirmation.
                </p>
                <div className="flex gap-2">
                  <span className="px-3 py-1 bg-green-50 text-green-700 text-[10px] font-bold rounded-md border border-green-100 uppercase">
                    FAST
                  </span>
                  <span className="px-3 py-1 bg-blue-50 text-blue-700 text-[10px] font-bold rounded-md border border-blue-100 uppercase">
                    SECURE
                  </span>
                </div>
              </div>
              ...
              >
                ...
                <p className="text-sm text-gray-500 mb-6">
                  Hand over cash to the agent. Receipt will be generated after
                  agent confirmation.
                </p>
                <div className="flex gap-2">
                  <span className="px-3 py-1 bg-gray-50 text-gray-700 text-[10px] font-bold rounded-md border border-gray-100 uppercase">
                    AGENT
                  </span>
                  <span className="px-3 py-1 bg-orange-50 text-[#fa8112] text-[10px] font-bold rounded-md border border-orange-100 uppercase">
                    OFFLINE
                  </span>
                </div>
              </div>
          ...
          <div className="w-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="bg-white border-2 border-[#E2E8F0] rounded-2xl p-6 shadow-sm h-fit">
                <h4 className="font-bold text-[#222222] mb-4 text-[16px]">
                  Summary
                </h4>
                <div className="space-y-3">
                  <div className="flex justify-between items-center py-2 border-b border-gray-50">
                    ...
                    <span className="font-bold text-[#222222]">
                      ₹
                      {Number(formData.payment.applicationFee || 120).toFixed(
                        2,
                      )}
                    </span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b border-gray-50">
                    <span className="text-gray-500 text-[14px]">
                      Member Add-ons ({(formData.members || []).length})
                    </span>
                    <span className="font-bold text-[#222222]">
                  ...
                  <div className="flex justify-between items-center py-4">
                    ...
                    <span className="font-bold text-[#fa8112] text-[24px]">
              ...
              <div className="flex flex-col gap-4">
                {paymentMethod === "online" ? (
                  <div className="bg-[#fa8112]/5 border-2 border-[#fa8112] rounded-2xl p-6 flex flex-col items-center text-center">
                    ...
                    <h5 className="font-bold text-[18px] text-[#222222] mb-2">
                      Instant Activation
                    </h5>
                    <p className="text-sm text-gray-500 mb-6">
                      Payment is processed securely via Cashfree. Your Ayush
                      Card will be activated instantly upon success.
                    </p>
                      ...
                      <>
                        {!orderId ? (
                          <button
                            ...
                            className="w-full py-4 bg-[#fa8112] hover:bg-[#e47510] text-white rounded-xl font-bold transition-all shadow-md flex items-center justify-center gap-2 text-[16px]"
                          >
                            {onlinePaymentLoading ? (
                              <Loader2 className="animate-spin" size={20} />
                            ) : (
                              "Pay ₹" + formData.payment.totalPaid
                            )}
                          </button>
                        ) : (
                          <div className="w-full space-y-3">
                            <div className="p-3 bg-blue-50 text-blue-700 rounded-xl text-[12px] font-medium border border-blue-100">
                              Waiting for payment... Click verify after
                              completing.
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                              <button
                                onClick={() => setOrderId(null)}
                                className="py-3 border-2 border-gray-200 rounded-xl font-bold text-gray-600 hover:bg-gray-50 transition-all"
                              >
                                Retry
                              </button>
                              <button
                                ...
                                className="py-3 bg-green-600 text-white rounded-xl font-bold hover:bg-green-700 flex items-center justify-center gap-2 transition-all"
                              >
                                {verifyLoading ? (
                  ...
                  <div className="bg-[#FAF3E1] border-2 border-[#fa8112] rounded-2xl p-6 flex flex-col gap-4">
                    <h5 className="font-bold text-[18px] text-[#222222]">
                      Agent Checklist
                    </h5>
                    ...
                    <div className="space-y-4 mt-2">
                      <label className="flex items-start gap-3 cursor-pointer group">
                        ...
                        <span className="text-[13px] font-medium text-gray-700">
                          Collect exactly ₹{formData.payment.totalPaid}.00 from
                          patient
                        </span>
                      </label>
                      <label className="flex items-start gap-3 cursor-pointer group">
                        ...
                        <span className="text-[13px] font-medium text-gray-700">
                          Verify original Aadhaar/PAN Card document
                        </span>
                      </label>
                      <label className="flex items-start gap-3 cursor-pointer group">
                        ...
                        <span className="text-[13px] font-medium text-gray-700">
                          Ensure all family members are accurately listed
                        </span>
                      </label>
                    </div>
                    <button
  ...
  const renderThermalReceipt = () => (
    ...
    >
      <div className="text-center border-b-2 border-dashed border-gray-300 pb-3 mb-3">
        <h2 className="font-bold text-[16px] uppercase tracking-tight">
          BKBS TRUST
        </h2>
        <p className="text-[10px] leading-none opacity-80">
          Ayushman Sewa Sansthan
        </p>
      </div>
      ...
      <div className="border-y border-dashed border-gray-300 py-2 mb-3">
        <div className="flex flex-col mb-1 text-[11px]">
          <span className="uppercase opacity-60 font-semibold mb-0.5">
            Applicant:
          </span>
      ...
      <div className="text-[9px] mt-6 border-t border-dashed border-gray-300 pt-3 text-left space-y-1">
        <div className="flex gap-1">
          <span className="font-bold uppercase opacity-60 shrink-0 italic">
            TXN ID:
          </span>
        ...
        <p className="text-center font-bold text-[11px] mt-4 uppercase tracking-wider border border-black py-1">
          Verified & Secure
        </p>
        <p className="text-center opacity-70 text-[8px] mt-2 italic">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-[#F8FAFC] rounded-2xl p-4 border border-[#E2E8F0] space-y-3">
            <div className="flex justify-between text-[13px]">
              ...
              <span className="font-bold text-[#222222] uppercase">
                {formData.applicantFirstName} {formData.applicantLastName}
              </span>
            </div>
            <div className="flex justify-between text-[13px]">
            ...
            <div className="flex justify-between text-[13px]">
              ...
              <span className="font-bold text-[#fa8112] uppercase tracking-wide">
                {paymentMethod}
              </span>
            </div>
          </div>
        ...
        <div className="px-8 py-5 border-b border-gray-100 flex justify-between items-center shrink-0">
          <div>
            <h2 className="text-[22px] font-bold text-[#222222]">
              Ayush Card Application
            </h2>
            <p className="text-[12px] text-gray-500 font-medium">
              BKBS Human Welfare & Social Trust
            </p>
          </div>
          <button
            onClick={() => navigate("/employee/health-card")}
